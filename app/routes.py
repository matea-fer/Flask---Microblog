#flash-to show the message to the user
#redirect- instructs the client web browser to
#automatically navigate to a different page
#Improving Field Validation sljedeci odlomak
...
def login():
    ...
    ### I just redirect to the index page.
    if current_user.is_authenticated:
        return redirect(url_for('index'))
    form = LoginForm()
    if form.validate_on_submit():
...
def edit_profile():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(_('Your messages have been saved.'))
